...
function get_gateway_configs($gateway)
{
    ...
    switch ($gateway->key) {
        ...
        case "aqayepardakht": {
                $configs['pin'] = $gateway->config('pin');
				$configs['callbackUrl'] = 'https://rivasit.com/orders/payment/callback/aqayepardakht';
                break;
            }
		case "sina_bank": {
                $configs['pin'] = $gateway->config('pin');
                $configs['terminalId'] = $gateway->config('terminalId');
                $configs['callbackUrl'] = 'http://pecpayment.in/confirm.php';
                break;
        }
    }
