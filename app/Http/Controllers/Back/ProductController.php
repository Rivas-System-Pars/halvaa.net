...
class ProductController extends Controller
{
    ...
    public function store(StoreProductRequest $request)
    {
        try{
			return DB::transaction(function()use ($request){
				...
				$this->_productGifts($request,$product);
        ...
        if ($request->brand) {
            $brand = Brand::firstOrCreate(
                [
					...
					'name' => $request->brand,
					'slug' => Str::slug($request->brand, '-', null),
				]
            );
    ...
    private function _productGifts(Request $request,$product)
    {
        if ($request->filled('gift_products') && is_array($request->gift_products) && count($request->gift_products)) {
            $product_id = $product->id;
            foreach ($request->gift_products as $gift_product) {
                $gift = ProductGift::query()->updateOrCreate([
                    'id' => Arr::get($gift_product, 'gift_id'),
                    'product_id' => $product_id,
                ], [
                    'quantity' => $gift_product['count'],
                    'product_id' => $product_id,
                ]);
                if (array_key_exists('products', $gift_product)) {
                    foreach ($gift_product['products'] as $product) {
                        if ($gift->products->contains('id', $product['product_id'])) {
                            $gift->products()->updateExistingPivot($product['product_id'], ['quantity' => $product['quantity']]);
                        } else {
                            $gift->products()->attach([$product['product_id'] => ['quantity' => $product['quantity']]]);
                        }
                    }
                }
            }
