...
class SettingController extends Controller
{
    ...
    public function updateInformation(Request $request)
    {
        ...
        $this->validate($request, [
			'installment_before_day' => 'nullable|numeric',
			'installment_day' => 'nullable|numeric',
			'installment_after_day' => 'nullable|numeric',
            ...
            'cash_discounts' => 'nullable|array',
            'cash_discounts.*' => 'array:min_amount,discount_amount,is_percent',
            'cash_discounts.*.min_amount' => 'required|numeric|min:0',
            'cash_discounts.*.discount_amount' => 'required|numeric',
            'cash_discounts.*.is_percent' => 'boolean',
            'per_purchases' => 'nullable|array',
            'per_purchases.*' => 'array:min_amount,discount_amount,is_percent',
            'per_purchases.*.min_amount' => 'required|numeric|min:0',
            'per_purchases.*.discount_amount' => 'required|numeric',
            'per_purchases.*.is_percent' => 'boolean',
            'discount_factor_rows' => 'nullable|array',
            'discount_factor_rows.*' => 'array:count,discount_amount,is_percent',
            'discount_factor_rows.*.count' => 'required|numeric|min:0',
            'discount_factor_rows.*.discount_amount' => 'required|numeric',
            'discount_factor_rows.*.is_percent' => 'boolean',
            'discount_priority' => ['nullable', 'array:cash_discount,discount_per_purchase,factor_rows,discount_products'],
            'discount_priority.cash_discount' => ['nullable','numeric', 'between:1,100'],
            'discount_priority.discount_per_purchase' => ['nullable','numeric', 'between:1,100'],
            'discount_priority.factor_rows' => ['nullable','numeric', 'between:1,100'],
            'discount_priority.discount_products' => ['nullable','numeric', 'between:1,100'],
        ]);
        $request->merge(['discount_priority'=>array_filter($request->discount_priority,'strlen')]);

        if (!array_key_exists('cash_discounts', $informations)) {
            $informations['cash_discounts'] = null;
        }
        if (!array_key_exists('per_purchases', $informations)) {
            $informations['per_purchases'] = null;
        }
        if (!array_key_exists('discount_factor_rows', $informations)) {
            $informations['discount_factor_rows'] = null;
        }
        ...
        foreach ($informations as $information => $value) {
            option_update($information, is_array($value) ? json_encode($value) : $value);
        }
