...
{
    use sluggable, Taggable, CategoryRepository;
    ...
    public function getLinkAttribute()
    {
        if ($this->type == 'productcat') {
			if($this->products->count() == 1){
				return route('front.products.show', $this->products->first());
			}else{
				return route('front.products.category-products', ['category' => $this]);
			}
        }
		if($this->type == 'postcat' && $this->slug == "دپارتمان-ها"){
			return "#";
		}
		if(is_null($this->category_id)){
			$posts = Post::published()->where('category_id', $this->id)->limit(2)->latest()->get();
			if($posts->count() == 1){
				return route('front.posts.show', $posts->first());
			}else{
        ...
        // return is_null($this->category_id) ? route('front.posts.category', ['category' => $this]) : url('/posts/'.$this->slug);
    }
