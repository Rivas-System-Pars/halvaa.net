...
{
    const SETTLEMENT_TYPE_CASH = "cash";

    const SETTLEMENT_TYPES = [
        self::SETTLEMENT_TYPE_CASH,
    ];

    protected $guarded = ['id'];

    public function pivotProductGifts(): HasMany
    {
        return $this->hasMany(ProductGiftOrder::class, 'order_id');
    }

    public function productGifts(): BelongsToMany
    {
        return $this->belongsToMany(Product::class, 'product_gift_order', 'order_id', 'product_gift_id')
            ->using(ProductGiftOrder::class)
            ->withPivot(['quantity', 'product_id', 'basket_id'])
            ->withTimestamps();
    }
    ...
    {
        return $this->hasMany(OrderReferralCategory::class);
    }
