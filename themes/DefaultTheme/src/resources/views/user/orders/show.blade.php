        ...
        <div class="row">
                ...
                <div class="col-12">
                    <div class="checkout-alert dt-sn mb-4">
                        ...
                        <div class="checkout-alert-title">
                            <h4> سفارش شما با شماره سفارش<span
                                    class="checkout-alert-highlighted checkout-alert-highlighted-success">{{ $order->id }}</span>
            ...
            <div class="col-12 mb-4">
                <div class="dt-sl dt-sn">
                    <div class="row table-draught px-3">
                        ...
                        <div class="col-md-6 col-sm-12">
                            ...
                            @if($order->status == 'unpaid')
								@if($order->installment)
									<form action="{{ route('front.orders.pre-pay', ['order' => $order]) }}" method="GET">
										<div class="row p-0">
						...
						@if($order->installment)
						<div class="col-md-6 col-sm-12">
                            <span class="title">پیش قسط:</span>
                            <span class="value">{{ number_format($order->installment->prepayment) }} تومان</span>
                        </div>
						@endif
                        <div class="col-md-6 col-sm-12">
                        ...
                        <div class="table-responsive mb-4">
                            <table class="table table-order table-order-details">
                                ...
                                <tr>
                                    ...
                                    <th>زمان پرداخت</th>
                                    <th>وضعیت</th>
									@if($order->installment->firstUnpaidItem)
                                    <th>عملیات</th>
									@endif
                                </tr>
                                </thead>
                                <tbody>
                                @foreach ($order->installment->items as $item)

                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ number_format($item->amount)." تومان" }}</td>
                                        <td>{{ number_format($item->fee)." تومان" }}</td>
                                        <td>{{ number_format($item->getTotalAmount())." تومان" }}</td>
                                        <td>{{ jdate($item->date)->format('%d %B %Y') }}</td>
                                        <td>
                                            <div class="@if($item->status == \App\Models\OrderInstallmentItem::STATUS_PAID) text-success @else text-muted @endif">{{ trans(ucfirst($item->status)) }}</div>
                                        </td>
										@if($order->installment->firstUnpaidItem)
                                        <td>
                                            @if($item->id == $order->installment->firstUnpaidItem->id && $order->status == 'paid')
                                                <a href="{{ route('front.orders.pay-installment',[$order->id,$item->id]) }}"
                                                   class="btn btn-info mb-2">پرداخت</a>
                                            @endif
                                        </td>
										@endif
                                    </tr>
                        ...
                        <table class="table table-order table-order-details">
                            ...
                            @foreach ($order->items as $item)
                                ...
                                <tr>
                                                ...
                                                @if ($item->get_price)
                                                    @foreach ($item->get_price->get_attributes as $attribute)
                                                        ...
                                                        @if ($attribute->group->type == 'color')
                                                            ...
                                                            <span class="order-product-color"
                                    ...
                                    <td>{{ $item->global_discount ? ($item->discount . '%')."(".number_format($item->global_discount)." تومان)" :($item->discount ? $item->discount . '%' : 0) }}</td>
                                    ...
                                    <td>
                                        @if ($item->product && $item->product->isDownload() && $item->get_price && $item->get_price->isDownloadable())
                                            <a href="{{ $item->get_price->downloadLink() }}"
                                               class="btn btn-success mb-2">دانلود</a>
                                        @endif
                                        <a href="{{ route('front.orders.return-products.show', ['order'=>$order->id,'product' => $item->product->id]) }}"
                                           class="btn btn-info mb-2">{{ $item->returned ? "اطلاعات مرجوعی" : "ثبت مرجوعی" }}</a>
                                        @if($item->product)
                                            <a href="{{ route('front.products.show', ['product' => $item->product]) }}"
            ...
            @if($order->productGifts->count())
                <div class="col-12 mt-4">
                    ...
                    <div class="dt-sl">
                        <div class="table-responsive">
                            <table class="table table-order table-order-details">
                                ...
                                <tbody>
                                @foreach ($order->productGifts as $item)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="{{ route('front.products.show', ['product' => $item->slug]) }}"
                                               class="btn btn-info mb-2">مشاهده</a>
                    ...
                    @if ($order->transactions->count())
                        <div
                            class="section-title text-sm-title title-wide no-after-title-wide mb-0 dt-sl px-res-1 mt-3">
                            <h2>جزئیات پرداخت</h2>
                        </div>
                        <section class="checkout-details dt-sl dt-sn mb-4 pt-2 pb-3 pl-3 pr-3">
                            <div class="row mt-3">
