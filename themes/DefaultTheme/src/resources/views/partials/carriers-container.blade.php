        ...
        <div class="row">
            <div class="checkout-time-table checkout-time-table-time">
                @foreach ($carriers as $carrier)
                    @if ($cart->canUseCarrier($carrier->id, $city_id)['status'])
                        <div class="col-12">
                            <div class="radio-box custom-control custom-radio form-group mb-0 pl-0 pr-3">
                                <input type="radio" class="custom-control-input form-control" name="carrier_id" id="carrier-{{ $carrier->id }}" value="{{ $carrier->id}}" {{ old('carrier_id', $carrier_id ?? '') == $carrier->id ? 'checked' : '' }}>
                ...
                @if (!$carrier_found)
                    <div class="col-12">
                        <div class="alert alert-danger">
                            روش ارسالی برای این آدرس و سفارش یافت نشد
                        </div>
                    </div>
                @endif
            </div>
