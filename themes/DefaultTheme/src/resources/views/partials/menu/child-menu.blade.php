    ...
    @switch($menu->type)
        @case('category')
            ...
            <li class="list-item list-item-has-children menu-col-1">
                <a class="nav-link nav-link-dropdown-width" href="{{ $menu->category->link }}">{{ $menu->category->title }}</a>
        ...
        @default
            ...
            <li class="list-item list-item-has-children menu-col-1">
                <a class="nav-link nav-link-dropdown-width" href="{{ $menu->link }}">{{ $menu->title }}</a>
                <ul class="sub-menu nav">
                    @foreach ($menu->childrenmenus as $childMenu)
                        @include('front::partials.menu.child-menu', ['menu' => $childMenu])
                    @endforeach
