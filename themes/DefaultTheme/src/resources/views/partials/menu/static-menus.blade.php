                ...
                <ul class="f-menu sub-menu nav">
                    @foreach ($productcats as $category)
                        <li class="{{ $loop->first ? 'active' : '' }}">
                            <a class="master-menu master-menu-width" href="{{ $category->link }}">{{ $category->title }}</a>
    ...
    @case('posts')
        @if($postcats->count())
            ...
            <li class="list-item list-item-has-children menu-col-1">
                <a class="nav-link nav-link-dropdown-width" href="{{ route('front.posts.index') }}">{{ $menu->title }}</a>
