...
class ProductController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        return view('front::products.search', compact('users'));
    }
...
public function ajax_search(Request $request)
{
    ...
    foreach ($users as $user) {
			//dd($user->profileImage->image);
        $results[] = [
            'title' => trim($user->first_name . ' ' . $user->last_name) ?: $user->username,
            'link' => route('front.user.showprofile', ['id' => $user->id]), // مسیر نمایش پروفایل
            'image' => $user->profileImage 
                ? asset($user->profileImage->image) // مسیر عکس پروفایل از رابطه
                : asset('/no-avatar.png'),
            'category' => '',
            'price' => $user->username,
        ];
